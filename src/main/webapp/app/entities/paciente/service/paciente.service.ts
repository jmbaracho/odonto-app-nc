...
type RestOf<T extends IPaciente | NewPaciente> = Omit<T, 'dataNascimento'> & {
...
export class PacienteService {
  ...
  protected convertDateFromClient<T extends IPaciente | NewPaciente | PartialUpdatePaciente>(paciente: T): RestOf<T> {
    return {
      ...paciente,
      dataNascimento: paciente.dataNascimento?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPaciente: RestPaciente): IPaciente {
    return {
      ...restPaciente,
      dataNascimento: restPaciente.dataNascimento ? dayjs(restPaciente.dataNascimento) : undefined,
    };
  }
